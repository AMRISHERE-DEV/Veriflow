...
from glass_ionomer.engine import ReleaseDecision, assure_sec_claim, verify_receipt
from glass_ionomer.lanes.financial import issue_trusted_companyfacts
from glass_ionomer.lanes.financial.models import companyfacts_are_trusted, financial_binding_subject
...
class SecAssuranceHelperTests(unittest.TestCase):
    ...
    def test_live_sec_fetch_is_byte_capped(self):
        resolver = importlib.import_module("glass_ionomer.lanes.financial.resolver")
        requested = []
        ...
        try:
            result = resolver.fetch_companyfacts(
                "320193", user_agent="Glass Ionomer test operator@example.test")
        finally:
    ...
    def test_live_sec_fetch_requires_real_operator_identity(self):
        ...
        self.assertIsNone(resolver.fetch_companyfacts(
            "320193", user_agent="Glass Ionomer set-your-email@example.com"))
    ...
    def test_live_sec_fetch_rejects_invalid_cik_before_network(self):
        ...
        try:
            result = resolver.fetch_companyfacts(
                "CIK-1", user_agent="Glass Ionomer test operator@example.test")
        finally:
            resolver.urllib.request.urlopen = original
        self.assertIsNone(result)
